...
const NonAlcoholicResult = ({id, name, price}) => {

    //  delete items 
    const deleteItem = async () => {
        await deleteDoc(doc(db, 'nonAlcohol', id))
    }
    ...
    const updateitems = async (e)=> {
        e.preventDefault();
        try {
            await updateDoc(doc(db, 'nonAlcohol', id), {name: newName, price: newPrice});
        } catch (error) {
            alert(error)
        }
    }
...
export default NonAlcoholicResult